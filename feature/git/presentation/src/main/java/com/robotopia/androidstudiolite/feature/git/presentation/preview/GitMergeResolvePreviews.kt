...
import com.robotopia.androidstudiolite.feature.git.presentation.project.logic.classifyConflictLinePaint
import com.robotopia.androidstudiolite.feature.git.presentation.project.logic.conflictTextFromDiffLines
...
private fun MergeResolveEditorPreview() {
    ...
    val conflictText = conflictTextFromDiffLines(lines)
    PreviewProjectGit(
        state = mergeConflictsState().copy(
            ...
            conflictLinePaint = classifyConflictLinePaint(conflictText),
        ),
    )
}
...
@Preview(showBackground = true, backgroundColor = GIT_PREVIEW_BG, widthDp = 360, heightDp = 640, name = "Merge · manual edit mid-resolve")
@Composable
private fun MergeManualEditMidResolvePreview() {
    val path = "app/src/main/java/Nav.kt"
    val edited = """
        package com.example.app
        ...
        <<<<<<< HEAD (main)
            Button(onClick = { /* current */ })
        =======
            OutlinedButton(onClick = { /* incoming */ })
        >>>>>>> feature/login
        }
    """.trimIndent()
    val paints = classifyConflictLinePaint(edited).toMutableList()
    // User edited the shared Text line — paint cleared for that row only.
    val editedLineIndex = edited.lines().indexOfFirst { it.contains("Hello from both") }
    if (editedLineIndex >= 0) paints[editedLineIndex] = ConflictLinePaint.None
    PreviewProjectGit(
        state = mergeConflictsState().copy(
            selectedDiffPath = path,
            diffTitle = "Nav.kt",
