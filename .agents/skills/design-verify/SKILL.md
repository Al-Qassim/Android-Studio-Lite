...
3. **Pass A — Cleanliness (page / phones as artifacts).**  
   ...
   **FAIL** full-bleed / near-full-phone **gray content slabs** (large rounded panel wrapping empty/list/form/**editor** under the top bar — looks like a gray card filling the UI, even with small side/bottom insets). Content should sit on the shell background; islands are tight (rows, fields, dialogs, menus, toasts)—not a second screen / code card inside the phone.  
